...
print("Initializing CamelBERT+BM25 test script... Please wait while heavy libraries load.", flush=True)

from backend.rag.retrieval.retriever_camelbert import retrieve as camelbert_retrieve
from bm25_rag.bm25_retriever import bm25_retrieve
from hybrid_rag.hybrid_retriever import reciprocal_rank_fusion
from hybrid_rag.reranker_camelbert import rerank_candidates
from qwen_rag.qwen_generator import qwen_generate

def hybrid_retrieve_camelbert_bm25(query: str, top_k: int = 5) -> list[dict]:
    fetch_k = 20
    
    # 1. BM25 Retrieval
    bm25_results = bm25_retrieve(query, top_k=fetch_k)
    
    # 2. CamelBERT Retrieval
    # camelbert_retrieve returns: {"id":..., "text":..., "metadata": {...}, "score":...}
    camelbert_raw = camelbert_retrieve(query, top_k=fetch_k)
    
    # Adapt camelbert results to match bm25's flat structure for easy fusion & generation
    camelbert_results = []
    for r in camelbert_raw:
        meta = r.get("metadata", {})
        adapted = {
            "id": r["id"],
            "score": r["score"],
        ...
        camelbert_results.append(adapted)
        
    # 3. Fuse results using Reciprocal Rank Fusion
    fused = reciprocal_rank_fusion(bm25_results, camelbert_results)
...
def main():
    ...
    output_file = PROJECT_ROOT / "answers_camelbert_bm25_final.txt"
    ...
    with open(output_file, "w", encoding="utf-8") as out:
        for i, q in enumerate(questions, 1):
            ...
            try:
                # Retrieve chunks using Hybrid RAG (CamelBERT + BM25)
                chunks = hybrid_retrieve_camelbert_bm25(q, top_k=5)
                
                # Generate answer using local Qwen2.5:7b
